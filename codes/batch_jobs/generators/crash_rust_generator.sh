...
BATCH_PATH="/home/3302/hf_py_code/compile/codes/batch_jobs/generators/"
...
do
    ...
    echo "make -f ${BATCH_PATH}crash_rust_makefile -j 64 \
    lang="${TYPE}" begin="${i}" \
    end="${STOP}"" >> ${js}
...
#    echo "ls -1 ${TYPE}/object_files >> \
#    ${PREFIX}${TYPE}/ps_${i}/object_files_list.txt" \
#    >> ${js}
    echo "find ${TYPE}/object_files >> \
    ${PREFIX}${TYPE}/ps_${i}/object_files_list.txt" \
    >> ${js}
    echo "cd .." >> ${js}
    echo "rm -r ps_${i}" >> ${js}
