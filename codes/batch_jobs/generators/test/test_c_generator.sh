...
#    echo "make -f ./makefile -j 64 \
    echo "make -f ${BATCH_PATH}makefile -j 64 \
    lang="${TYPE}" begin="${i}" \
    end="${STOP}"" >> ${js}
    echo "mkdir -p ${PREFIX}${TYPE}/ps_${i}" >> ${js}
#    echo "(find ${TYPE}/textseg_sizes | xargs cat) \
#    >> ${PREFIX}${TYPE}/ps_${i}/text_segments.csv" \
#    >> ${js}
...
#    echo "(find ${TYPE}/instructions_counts | xargs cat) \
#    >> ${PREFIX}${TYPE}/ps_${i}/instructions.csv" \
#    >> ${js}
    while true; do
